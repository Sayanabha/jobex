...
export function LoadingScene({
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-[28px] border border-white/10 bg-white/5 p-6", className)}>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(0,255,194,0.12),transparent_34%),radial-gradient(circle_at_bottom_left,rgba(255,149,77,0.12),transparent_30%)]" />
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {steps.map((step, index) => (
            <div
              key={step}
              className={cn(
                "rounded-2xl border border-white/10 px-4 py-3 text-sm text-muted-foreground transition-all duration-300",
